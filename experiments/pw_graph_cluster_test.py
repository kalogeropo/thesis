...
if __name__ == '__main__':
    ...
    total_exps = len(cluster_sizes) * len(percentages) * 2 # (2 is the number of models)
    current_exp = 0
    for perc in percentages:
        for cluster_size in cluster_sizes:
            ...
            print(f'Experiment: (Window-{perc} - {cluster_size})')
        
            ################ GRAPHICAL SET BASED Window Per WITH PRUNING (PGSBW) ####################  
            pgsbw_model = PGSBW(col, window=perc, clusters=cluster_size).fit(queries)
            ...
            experiment[f'PGSBW-{perc}-{cluster_size}'] = pre


            ############### CONCEPTUALIZED GRAPHICAL SET BASED Window ####################
            con_gsbw_model = ConGSBWindow(col, window=perc, clusters=cluster_size).fit(queries)
            ...
            # concatenate new experiment
            experiment[f'CGSBW-{perc}-{cluster_size}'] = pre
    ...
    writer.write_to_excel(sheet_name='pw-graph-clusters', dataframe=experiment)