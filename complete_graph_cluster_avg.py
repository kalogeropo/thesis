...
if __name__ == '__main__':
    ...
    for cluster_size in cluster_sizes:
        for coeff in edge_coeffs:
            ...
            pgsb_model = PGSB(col, clusters=cluster_size, condition={'edge': 8*coeff}).fit(queries)
            ...
            experiment[f'PGSB-{cluster_size}-{coeff}'] = pre


            ############### CONCEPTUALIZED GRAPHICAL SET BASED ####################
            con_gsb_model = ConGSB(col, clusters=cluster_size, cond={'edge': 8*coeff}).fit(queries)
            ...
            experiment[f'CGSB-{cluster_size}-{coeff}'] = pre
